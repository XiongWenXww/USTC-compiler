...
float	{return yy::sysyfRarser::make_FLOAT(loc);}
string	{return yy::sysyfRarser::make_STRING(loc);}
bool	{return yy::sysyfRarser::make_BOOL(loc);}
...
{FloatConst}	{
                            std::string float = yytext;
                            unsigned sum = 0;
                            int len = float.size();
                            for(int i = 1;i < len&&float[i]!='.';i++){
                                auto a = float[i];
                                sum  = sum * 10 + a - 48;
                            }
	            for(i=i+1;i<len;i++){
		...
		sum = sum * 10 + a - 48;
                            ...
                            return yy::sysyfParser::make_FLOATCONST(int(sum),loc);
                        }		
{Identifier}            {return yy::sysyfParser::make_IDENTIFIER(yytext, loc);}
{SingleLineComment}	{return yy::sysyfParser::make_COMMENT}
{MultilineComment}		{return yy::sysyfParser::make_COMMENT}
