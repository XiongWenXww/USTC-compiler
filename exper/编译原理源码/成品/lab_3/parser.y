...
%token EXT_DEF_LIST EXT_VAR_DEF FUNC_DEF FUNC_DEC EXT_DEC_LIST PARAM_LIST PARAM_DEC VAR_DEF DEC_LIST DEF_LIST COMP_STM STM_LIST EXP_STMT IF_THEN IF_THEN_ELSE DPLUS_BEFORE DPLUS_AFTER DMINUS_BEFORE DMINUS_AFTER DIMENSION DIMENSION_LIST ARRAY ARRAY_LIST ARRAY_1 ARRAY_N PROGRAM
...
program: ExtDefList    {$$=mknode(1,PROGRAM,yylineno,$1);display($$,0);semantic_Analysis0($1);}     //显示语法树,语义分析  魔改semantic_Analysis0($1); 
         ; 
ExtDefList: {$$=NULL;}
...
ExtDef:   Specifier ExtDecList SEMI   {$$=mknode(2,EXT_VAR_DEF,yylineno,$1,$2);}   //该结点对应一个外部变量声明
         ...
         | error SEMI   {$$=NULL;}
         ;
Specifier:  TYPE    {$$=mknode(0,TYPE,yylineno);strcpy($$->type_id,$1);if(!strcmp($1,"int")) $$->type=INT;if(!strcmp($1,"float")) $$->type=FLOAT;if(!strcmp($1,"char")) $$->type=CHAR;}   
           ;      
...
VarDec:  ID          {$$=mknode(0,ID,yylineno);strcpy($$->type_id,$1);}   //ID结点，标识符符号串存放结点的type_id
		| ID DimensionList	{$$=mknode(1,DIMENSION,yylineno,$2);strcpy($$->type_id,$1);}//数组声名
         ;
...
ArrayListN:	  ArrayListN COMMA ArrayList1 {$$=mknode(2,ARRAY_N,yylineno,$1,$3);}
			...
			| ArrayList			%prec LCOMMA {$$=mknode(1,ARRAY_1,yylineno,$1);}//一维数组
			;
